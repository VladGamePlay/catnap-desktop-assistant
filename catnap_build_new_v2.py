...
APP_VERSION = "1.6.0" # Новая версия
APP_YEAR = "2025"
...
class CatNapApp(ctk.CTk):
    def __init__(self):
        ...
        self.sprite_label.bind("<ButtonRelease-1>", self.handle_mouse_release_on_sprite)
    ...
    def start_drag(self, event):
        # Существующая логика для перетаскивания окна
        self.x_drag = event.x
        self.y_drag = event.y

        # --- Новая логика для "поглаживания" ---
        self.is_mouse_pressed_on_sprite = True
        self.mouse_press_start_pos = (event.x, event.y) # Запоминаем начальную позицию клика НА СПРАЙТЕ
        self.pet_stroke_count = 0 # Сбрасываем счетчик движений
        ...
        # Запускаем таймер, который сбросит попытку "поглаживания", если не будет достаточно движений
    ...
    def do_drag(self, event):
        # --- Логика для "поглаживания" ---
        # Проверяем, активно ли нажатие для поглаживания и не слишком ли далеко ушел курсор
        current_time = time.time()
        if self.is_mouse_pressed_on_sprite:
            dx = abs(event.x - self.mouse_press_start_pos[0])
            dy = abs(event.y - self.mouse_press_start_pos[1])

            # Если смещение слишком большое, это перетаскивание, а не поглаживание
            if dx > PET_MAX_DRAG_DISTANCE_PX or dy > PET_MAX_DRAG_DISTANCE_PX:
                # print(f"Drag detected (dx={dx}, dy={dy}), resetting pet attempt.") # Для отладки
                if self.pet_detection_timer_id: # Отменяем таймер, так как это уже не поглаживание
                    self.after_cancel(self.pet_detection_timer_id)
                    self.pet_detection_timer_id = None
                self.is_mouse_pressed_on_sprite = False # Больше не считаем это попыткой погладить
                self.pet_stroke_count = 0
                # Переходим к стандартному перетаскиванию окна (код ниже)
            else:
                # Это короткое движение, похожее на поглаживание
                self.pet_stroke_count += 1
                # print(f"Stroke count: {self.pet_stroke_count}") # Для отладки

                if self.pet_stroke_count >= PET_STROKE_THRESHOLD and \
                (current_time - self.last_pet_time) > PET_COOLDOWN_S:
                    # print("Petting detected!") # Для отладки
                    self._react_to_petting()
                    self.last_pet_time = current_time
                    self.pet_stroke_count = 0 # Сбрасываем для следующего раза
                    
                    # Успешное поглаживание, отменяем таймер обнаружения
                    if self.pet_detection_timer_id:
                        self.after_cancel(self.pet_detection_timer_id)
                        self.pet_detection_timer_id = None
                    self.is_mouse_pressed_on_sprite = False # Завершаем текущую попытку поглаживания
                    # return # Можно раскомментировать, если не хотим, чтобы после поглаживания окно чуть-чуть сдвигалось
                            # Но тогда нужно убедиться, что is_mouse_pressed_on_sprite = False сбросит и перетаскивание окна

        # --- Существующая логика перетаскивания окна ---
        # Важно: эта часть должна выполняться, даже если было поглаживание,
        # но is_mouse_pressed_on_sprite может быть уже False.
        # Перетаскивание окна происходит на основе self.x_drag и self.y_drag,
        # которые устанавливаются в start_drag.
        # Если is_mouse_pressed_on_sprite стало False из-за поглаживания,
        # то следующие движения мыши уже не будут считаться для поглаживания,
        # но могут продолжать немного двигать окно, если кнопка еще зажата.
        # Это поведение можно настроить.
        # Для более четкого разделения, можно использовать отдельный флаг для перетаскивания,
